    ...
    for row in soup.find_all("li", {"class": "xans-record-"}):
        if row.find("div", attrs={"class": "thumbnail"}) is not None:
            ...
            if ul_price:
                ...
                if fixed_span:
                    article["price"] = _extract_decimal_from_text(
                        fixed_span.get_text(" ", strip=True)
                    )
                else:
                    # Fallback: 할인판매가
                    discount_li = ul_price.find("li", attrs={"rel": "할인판매가"})
                    if discount_li:
                        discount_span = discount_li.find(
                            "span",
                            style=re.compile(
                                r"font-size:\s*12px;\s*color:\s*#555555", re.IGNORECASE
                            ),
                        )
                        if discount_span:
                            article["price"] = _extract_decimal_from_text(
                                discount_span.get_text(" ", strip=True)
                            )
                        else:
                            article["price"] = _extract_decimal_from_text(
                                discount_li.get_text(" ", strip=True)
                            )
                    else:
                        # Fallback: 최적할인가
                        best_li = ul_price.find("li", attrs={"rel": "최적할인가"})
                        if best_li:
                            best_span = best_li.find(
                                "span",
                                style=re.compile(
                                    r"font-size:\s*12px;\s*color:\s*#555555", re.IGNORECASE
                                ),
                            ...
                            if best_span:
                                article["price"] = _extract_decimal_from_text(
                                    best_span.get_text(" ", strip=True)
                                )
                            else:
                                article["price"] = _extract_decimal_from_text(
                                    best_li.get_text(" ", strip=True)
                                )
