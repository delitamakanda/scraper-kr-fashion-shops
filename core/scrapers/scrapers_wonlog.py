    ...
    for attempt in range(max_retries):
        try:
            ...
            if response.status_code == 200:
                ...
                logger.warning(
                    f"Attempt #{attempt + 1} failed. Status code: {response.status_code}"
                )
...
def connect_to_base(browser, max_attempts=5):
    ...
    for attempt in range(max_attempts):
        try:
            ...
            WebDriverWait(browser, 20).until(EC.presence_of_element_located((By.ID, "contents")))
...
def get_load_time(article_url):
    try:
        ...
        response = requests.get(article_url, headers=headers, stream=True, timeout=5)
        # get page load time
