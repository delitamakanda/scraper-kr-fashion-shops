...
class APIRoot(View):
    ...
    def get(request):
        data = {
            "products": {"count": Product.objects.all().count(), "url": reverse("products_api")},
            "newsletter": {
                "count": UserMailing.objects.all().count(),
                "url": reverse("create_signup_mail"),
            },
            "weather": {"url": reverse("weather_api")},
            "latest_sync_job": {"url": reverse("latest_sync_job_api")},
            "latest_sync_job_five_api": {"url": reverse("latest_sync_job_five_api")},
            "status": {"url": reverse("healthcheck:system_status")},
            "ping": {"url": reverse("healthcheck:ping_status", args=["localhost"])},
            "check_alerts": {"url": reverse("healthcheck:check_alerts")},
            "alerts": {"url": reverse("healthcheck:get_all_alerts")},
            "metrics": {"url": reverse("healthcheck:get_metrics_by_host", args=[1])},
        }
...
class ProductListApiView(View):
    ...
    def get(request):
        ...
        request_uri = (
            request.build_absolute_uri().replace("?page=" + str(ps["page"]), "").replace("&q=", "")
        )
        return JsonResponse(
            {
                ...
                "next": request_uri
                + "?page="
                + str(ps["products_page"].next_page_number())
                + "&q="
                + ps["search_query"]
                if ps["products_page"].has_next()
                else None,
                "previous": request_uri
                + "?page="
                + str(ps["products_page"].previous_page_number())
                + "&q="
                + ps["search_query"]
                if ps["products_page"].has_previous()
                else None,
